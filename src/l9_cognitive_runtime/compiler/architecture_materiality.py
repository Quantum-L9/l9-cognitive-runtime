...
- context signals: governed architecture signals proven by the typed
  ...
  INV-CTX-006 a raw hint cannot by itself establish architecture truth. To prove
  a signal, a host supplies a provenance-backed ``GovernedConstraint`` whose
  ``constraint_id`` is the signal name;
...
def assess_materiality(
    ...
    # Governed signals only. Every entry here traces to a discovery item whose
    # digest is recorded in ``discovery.selected_item_digests``.
    governed_signals = list(discovery.architecture_signal_refs) if discovery else []

    triggers: set[str] = set()
    evidence: list[str] = []
    for signal in governed_signals:
