...
Three rules shape the whole module and are easy to break by accident:
...
2. Semantic keys are defined by the item's *kind*, never by the caller
   ...
   key fails closed.
3. Snapshot candidates enter with empty ``selected_because``; only the
...
class ContextItemIdentity(ArtifactModel):
    ...
    @field_validator("item_id", "semantic_key")
    @classmethod
    ...
    def _validate_identity(self) -> ContextItemIdentity:
        ...
        expected = self.expected_semantic_key()
        if expected is not None and self.semantic_key != expected:
            raise ValueError(
                f"semantic_key must equal the {self.context_kind.value} recipe: {expected!r}"
            )
...
class ContextRequirement(ArtifactModel):
    ...
    @field_validator("scope_refs", "required_semantic_keys")
    @classmethod
    def _canonical_lists(cls, value: list[str]) -> list[str]:
        return _sorted_unique(value)
